...
export class BinaryTree {
    ...
    insertNode(newNodeData: number): void {
        let node = new BinaryTreeNodeModel({
            data: newNodeData,
        });
                ...
                if (newNodeData > currentNode.data) {
                    if (!currentNode.right) {
                        ...
                        if (currentNode.parent) {
                            this.updateViewPortOfAllParents(currentNode, 'rightViewPort', currentNode.right.rightViewPort);
                        } else {
                    ...
                    if (!currentNode.left) {
                        ...
                        if (currentNode.parent) {
                            this.updateViewPortOfAllParents(currentNode, 'leftViewPort', currentNode.left.leftViewPort);
                        } else {
    ...
    updateViewPortOfAllParents(parentNode: BinaryTreeNodeModel | any, viewport: string, value: number): void {
        if (parentNode.parent) {
            if (viewport === 'leftViewPort' && parentNode[viewport] > value
